...
namespace ns
{
    namespace VNLightComponents
    {
        ...
        void Dialogue::SetNeko(Persona *info)
        {
            if (info)
            {
                nekoInfo = info;
        ...
        {
            spriteLoaded = characterLoaded = false;
            if (novelCharacter && novel)
            {
                imagePath = L"Data/Characters/" + novelCharacter->name + L".png";
                sf::Texture* texture = ic::LoadTexture(imagePath);
                if ((spriteLoaded = texture))
                {
                    sprite.setTexture(*texture, true);
                    relScale = (static_cast<float>(gs::relativeHeight)/static_cast<float>(texture->getSize().y)) * novelCharacter->novelScale;
            ...
            characterLoaded = novelCharacter ? true : spriteLoaded;
            
            if (novel && !characterLoaded)
            {
                if (sendMessageBack != noMessage) novel->UnHold(this);
                novelSystem->PopComponent(this);
            }
        ...
        void Character::Update(const sf::Time& elapsedTime)
        {
            switch (mode)
            {
                case appearing:
                    ...
                    else persona->setPersonAlpha(alpha);
                    gs::requestWindowRefresh = true;
                    break;
        ...
        void Novel::Update(const sf::Time& elapsedTime)
        {
            ...
            while (!eof && onHold.empty())
            {
                ...
                else if (nss::Command(command, L"show "))
                {
                    ...
                    {
                        std::unordered_map<std::wstring, NovelCharacter*>::iterator neko;
                        if (possibleName != L"neko") neko = CharacterLibrary::library.find(possibleName);
                        if (possibleName == L"neko" || neko != CharacterLibrary::library.end())
                        {
                            auto* component = layers.PrioritizeComponent<Character>(5000);
                            component->novel = this;
                            if (possibleName != L"neko") component->novelCharacter = (*neko).second;
                            else component->persona = nekoPersona;
                ...
                else if (nss::Command(command, L"hide "))
                {
                    ...
                    if (possibleName.length() != 0)
                    {
                        ...
                        for (auto n : layers.objects)
                        {
                            ...
                            if ((c = dynamic_cast<Character*>(n)))
                            {
                                if ((c->persona && possibleName == L"neko") || (c->novelCharacter && c->novelCharacter->name == possibleName))
                                {
                                    if (sendMessageBack != noMessage) OnHold(c);
                                    switch (sendMessageBack)
                                    {
