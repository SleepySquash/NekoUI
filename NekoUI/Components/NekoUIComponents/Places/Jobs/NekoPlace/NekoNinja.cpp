...
namespace NekoNinja
{
    ...
    void SceneBackground::Resize(const unsigned int& width, const unsigned int& height)
    {
        if (spriteLoaded)
        {
    ...
    void GUIOverlay::Destroy() { ic::DeleteImage(L"Data/Jobs/NekoPlace/UI/pause_button.png"); ic::DeleteImage(L"Data/Jobs/NekoPlace/UI/heart.png"); }
    void GUIOverlay::Init()
    {
        ...
        sf::Texture* texture = ic::LoadTexture(L"Data/Jobs/NekoPlace/UI/pause_button.png");
        if (texture) pauseSprite.setTexture(*texture);
        
        texture = ic::LoadTexture(L"Data/Jobs/NekoPlace/UI/groping-hand.png");
        if (texture) handSprite.setTexture(*texture);
        handSprite.setOrigin(170, 136);
        
        texture = ic::LoadTexture(L"Data/Jobs/NekoPlace/UI/heart.png");
        if (texture)
        {
        ...
        if (Player::self->neko)
        {
            nekoButton.setTexture(L"Data/Jobs/NekoPlace/Neko/Avatar/" + Player::self->neko->name + L".png");
            ...
            nekoButShape.setFillColor(sf::Color(Player::self->neko->color.r, Player::self->neko->color.g, Player::self->neko->color.b, 150));
    ...
    void GUIOverlay::Resize(const unsigned int& width, const unsigned int& height)
    {
    ...
    void GUIOverlay::Draw(sf::RenderWindow* window)
    {
        ...
        if (control->isGameOver)
        {
            if (drawResults)
            {
                ...
                helperText.setCharacterSize((unsigned int)(64 * gs::scale));
                helperText.setString(L"Заработал " + std::to_wstring(control->moneyEarned) + L" рублей!");
            ...
            else if (drawNewNeko)
            {
                ...
                {
                    if (ic::FindTexture(L"Data/Jobs/NekoPlace/Neko/Avatar/" + neko->name + L".png"))
                        newNekoSprite.setTexture(*ic::FindTexture(L"Data/Jobs/NekoPlace/Neko/Avatar/" + neko->name + L".png"), true);
                    else if (ic::FindTexture(L"Data/Jobs/NekoPlace/Neko/Chibi/" + neko->name + L".png"))
                        newNekoSprite.setTexture(*ic::FindTexture(L"Data/Jobs/NekoPlace/Neko/Chibi/" + neko->name + L".png"), true);
    ...
    void GUIOverlay::GameOver()
    {
        ...
        // if (newNekoAvailable && !Player::self->roomUnlocked) { control->entity->AddComponent<Popup>(L"Новая кошкодевочка!", L"Теперь Вам доступна комната с кошкодевочками! Нажмите в главном меню на Чоколу справа, чтобы попасть туда."); }
    ...
    void NekoObject::Init()
    {
        ...
        else
        {
            ...
            for (; last < nl::self->neko.size(); ++last)
                if (nl::self->neko[last]->possibility < possibility) break;
            
            do { nekoIndex = rand() % last; } while (nl::self->neko[nekoIndex]->difficultyAvailable > control->currentDifficulty);
            nekoInfo = nl::self->neko[nekoIndex];
        ...
        sf::Texture* texture = ic::LoadTexture(L"Data/Jobs/NekoPlace/Neko/Chibi/" + nekoInfo->name + L".png");
        if (texture)
        {
    ...
    void NekoObject::Destroy()
    {
        ...
        if (nekoInfo)
        {
            ic::DeleteImage(L"Data/Jobs/NekoPlace/Neko/Chibi/" + nekoInfo->name + L".png");
            sc::DeleteSound(L"Data/Jobs/NekoPlace/Neko/Nya/" + nekoInfo->name + L".ogg");
        }
    }
    ...
    void NekoObject::Tame()
    {
        ...
        else
        {
            ...
            sf::SoundBuffer* buffer = sc::LoadSound(L"Data/Jobs/NekoPlace/Neko/Nya/" + nekoInfo->name + L".ogg");
            if (buffer)
            {
    ...
    void Controller::Destroy()
    {
        ...
        if (needsInit)
        {
            for (int i = 0; i < nl::self->neko.size(); ++i)
            {
                ic::SetDestroyable(L"Data/Jobs/NekoPlace/Neko/Chibi/" + nl::self->neko[i]->name + L".png", true);
                sc::SetDestroyable(L"Data/Jobs/NekoPlace/Neko/Nya/" + nl::self->neko[i]->name + L".ogg", true);
            }
            ic::SetDestroyable(L"Data/Jobs/NekoPlace/UI/heartsshape.png", true);
        }
        
        gui.Destroy();
        
        if (Player::self) { delete Player::self; Player::self = nullptr; }
    }
    void Controller::Init()
    {
        if (Player::self) delete Player::self;
        ...
        if (needsInit)
        {
            /*ic::PreloadTexture(L"Data/Jobs/NekoPlace/UI/heartsshape.png", 0, false);
            ic::PreloadTexture(L"Data/Jobs/NekoPlace/UI/heart.png", 0, false);
            ic::PreloadTexture(L"Data/Jobs/NekoPlace/UI/groping-hand.png", 0, false);
            ic::PreloadTexture(L"Data/Jobs/NekoPlace/UI/pause_button.png", 0, false);*/
            for (int i = 0; i < nl::self->neko.size(); ++i)
            {
                ic::PreloadTexture(L"Data/Jobs/NekoPlace/Neko/Chibi/" + nl::self->neko[i]->name + L".png", 0, false);
                //TODO: sc::PreloadSound(L"Data/Neko/Nya/" + nl::neko[i]->name + L".ogg", 0, false);
            }
        }
        
        firstNeko = new NekoObject();
        NekoObject::tamedArray = new unsigned int[nl::self->neko.size()]();
        if (Player::self->neko && Player::self->activeNekoUnlocked) {
            ability = Player::self->neko->ability;
    ...
    void Controller::Resize(const unsigned int& width, const unsigned int& height)
    {
    ...
    void Controller::ReceiveMessage(MessageHolder& message)
    {
    ...
    void Controller::GameOver()
    {
        ...
        if (gui.prevLevel < 35 && Player::self->level >= 35) { ++Player::self->passiveNekosUnlocked;
            entity->system->SendMessage({"NotUI :: Popup", new NotificationHolder(L"Вы достигли 35-го уровня!", L"Теперь Вы можете добавить в отряд на прогулку третью кошкодевочку с пассивной способностью!")}); }
        if (gui.prevLevel < 25 && Player::self->level >= 25) { ++Player::self->passiveNekosUnlocked;
            entity->system->SendMessage({"NotUI :: Popup", new NotificationHolder(L"Вы достигли 25-го уровня!", L"Теперь Вы можете добавить в отряд на прогулку вторую кошкодевочку с пассивной способностью!")}); }
        if (gui.prevLevel < 16 && Player::self->level >= 16) { ++Player::self->passiveNekosUnlocked;
            entity->system->SendMessage({"NotUI :: Popup", new NotificationHolder(L"Вы достигли 16-го уровня!", L"Теперь Вы можете добавить в отряд на прогулку ещё одну кошкодевочку с пассивной способностью!")}); }
        if (gui.prevLevel < 6 && Player::self->level >= 6) {
            if (Player::self->neko) ability = Player::self->neko->ability;
            entity->system->SendMessage({"NotUI :: Popup", new NotificationHolder(L"Вы достигли 6-го уровня!", L"Теперь Вы можете использовать активную способность кошкодевочки!")}); }
        
        gui.GameOver();
        /*for (unsigned int i = 0; i < nl::neko.size(); ++i)
            if (NekoObject::tamedArray[i]) nl::neko[i]->tamed = true;*/
        Player::self->SaveNekos();
        if (gui.newNekoAvailable) { NekoDatabaseCollection_SortNeko(); }
        ...
        if (score > Player::self->topScore || maxStrike > Player::self->topStrike || score != 0)
        {
            ...
            if (!base::FileExists(Player::self->scoresPath)) base::CreateDirectory(base::GetFolderPath(Player::self->scoresPath));
    ...
    void Controller::ResetGame()
    {
        ...
        for (auto& n : Player::self->passiveNeko) if (n->ability) n->ability->OnEnd();
        ...
        for (unsigned int i = 0; i < nl::self->neko.size(); ++i) NekoObject::tamedArray[i] = 0;
        ...
        for (auto& n : Player::self->passiveNeko) if (n->ability) n->ability->OnAction();
    ...
    void Controller::ReturnToMenu()
    {
        ...
        if (event) { event->OnEnd(); delete event; event = nullptr; }
        gs::isPause = false;
